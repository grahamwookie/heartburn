...
;var $ = jQuery;
var $selectedItem = $('div.sidebar-navigation__list > ol > li > a');
...
$document.on('click', 'a', function(e){
  ...
  // href must be a hash followed by some characters
  // href value must match an element ID
  if (href.search('#.') === 0 && $(href).length > 0) {
    ...
    Webtrends.multiTrack({
      element: this,
      argsa: ["DCSext.Anchor", href, "WT.dl", "121"]
    });
  }
});
...
function onScroll(event){
  ...
  $selectedItem.each(function () {
      ...
      if ($refElement.position().top <= scrollPos && $refElement.position().top + $refElement.parent().height() > scrollPos) {
          $('.selected').removeClass("selected");
