...
;$( document ).ready(function() {
  ...
  if (isFirefox) {
    ...
    var ffver = ffmatch ? parseInt(ffmatch[1]) : 0;
    isOldFirefox = isFirefox && ffver < 55;
  }

  var isOpera = (!!window.opr && !!opr.addons) || !!window.opera || navigator.userAgent.indexOf(' OPR/') >= 0;
  var opera_version = 0
  if (isOpera) {
    var v = navigator.userAgent.match(/OPR\/(\S+)/);
    var full_version = v[0].split('/');
    var version_array = full_version[1].split('.')
    opera_version = version_array[0]
  }

  var isOldOpera = isOpera && opera_version < 47;
  ...
  var isOldChrome = isChrome && major_version < 60

  var old_browser = isIE || isEdge || isOldFirefox || isOldChrome || isOldOpera || isSafari

  if (old_browser) {
    $( ".main-nav__page-menu > li" ).focusin(function() {
      $(".main-nav__page-menu--open").removeClass("main-nav__page-menu--open");
      if ($(window).width() > 1023) {
        $(this).addClass("main-nav__page-menu--open");
        $(this).find("a").addClass("main-nav__page-menu--open");
      }
    });
    $( ".main-nav__page-menu > li" ).focusout(function() {
      if ($(this).find("a:focus").length == 0 && !isOldFirefox) {
        $(this).removeClass("main-nav__page-menu--open");
      }
    });
    $("a").focusin(function() {
      if(!$(this).hasClass("main-nav__page-menu--open")) {
        $(".main-nav__page-menu--open").removeClass("main-nav__page-menu--open");
      }
    });
  }
    ...
    if (form) {
        ...
        form.addEventListener("submit", function(event) {
            ...
            var checks = [
                ...
                ["agree_terms", "ds_tcError", "Please accept terms and conditions", "ds_confirmGroup"]
            ];
